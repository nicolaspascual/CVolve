...
def get_skills_sorted_by_distance(user_id, offer_id, limit=None, return_distances=False):
    limit_clause = f'LIMIT {limit}' if limit else ''
    distance_clause = ", main_jobofferskilldistance.distance" if return_distances else ''
    return UserSkill.objects.raw(
        f"""
        SELECT main_userskill.* {distance_clause}
